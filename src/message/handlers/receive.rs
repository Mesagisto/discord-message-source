...
use mesagisto_client::{cache::CACHE, data::{Packet, message::{Message, MessageType}}, db::DB};
use serenity::model::{channel::{MessageReference, AttachmentType}, id::{ChannelId, MessageId}};
  ...
  for single in message.chain {
    ...
    match single {
        ...
        let receipt = if let Some(reply_to) = &message.reply {
          ...
          match local_id {
            Some(local_id) => {
              ...
              target.send_message(&**BOT_CLIENT, |m|
                m.reference_message(refer)
                  .content(content)
              ).await
            }
            None => {
              target.send_message(&**BOT_CLIENT, |m| m.content(content)).await
            }
          }
        } else {
          target.send_message(&**BOT_CLIENT, |m|
      ...
      MessageType::Image { id, url } => {
        ...
        let receipt = target.send_message(&**BOT_CLIENT, |m|{
